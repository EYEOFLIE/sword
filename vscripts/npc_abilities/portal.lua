...
function TeleportAllHeroesToAct1(caster, relatedBeaconPortal)
    ...
    for i = 1, #MAIN_HERO_TABLE, 1 do
        ...
        if not Beacons:IsHeroInCorrectLocation(hero, relatedBeaconPortal) then
            ...
            Events:TeleportUnit(hero, Events.portal.teleportLocation, ability, caster, 1.2)
            hero.inTown = false
        end
    end
            ...
            if unit:HasModifier("modifier_activating_beacon") then
                if caster.type == "wave" then
                    ...
                    function()
                        ...
                        Events:wave_redirect() 
                        TeleportAllHeroesToAct1(Events.portal, relatedBeaconPortal) 
                    end) 
...
function beaconDungeon(caster, unit)
    ...
    CustomGameEventManager:Send_ServerToAllClients("DungeonVote", {dungeonName = caster.dungeon, dungeonImage = caster.dungeonImagePath, recommendedLevel = caster.recommendedLevel, playerID = unit:GetPlayerOwnerID(), hero = unit:GetClassname(), numPlayers = #MAIN_HERO_TABLE, hero1 = hero1, player1 = player1, hero2 = hero2, player2 = player2, hero3 = hero3, player3 = player3, hero4 = hero4, player4 = player4})
