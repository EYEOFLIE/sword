...
-- 以下数值修改为自己想要的平衡性常数即可
local HP_PER_STR = 30 -- 力量 - 血量
local HP_REGEN_PER_STR = 0.1 -- 力量 - 生命恢复
local MANA_PER_INT = 10 -- 智力 - 魔法
local MANA_REGEN_PER_INT = 0.1 -- 智力 - 魔法回复
local ARMOR_PER_AGI = 0.05 -- 敏捷 - 护甲
local ATKSPD_PER_AGI = 0.5 -- 敏捷 - 攻击速度
...
local DEFAULT_ARMOR_PER_AGI = 0.142857
...
function equilibrium_constant:GetModifierBonusStats_Intellect( params )
    if IsServer() then
        local owner = self:GetParent()
        if owner:GetUnitName()=="npc_dota_hero_dragon_knight" then
...
function equilibrium_constant:GetTexture( params )
