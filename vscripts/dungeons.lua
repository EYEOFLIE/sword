...
function Dungeons:SpawnDungeonUnit(unitName, spawnPoint, quantity, minDrops, maxDrops, aggroSound, fv, isAggro, special)
	for i = 1, quantity, 1 do
		...
		if ability then
			ability:SetLevel(1)
			ability:ApplyDataDrivenModifier(unit, unit, "modifier_dungeon_thinker_creep", {duration = 99999})
		end
		if aggroSound then
	    				...
	    				Timers:CreateTimer(1.5, function()
							...
							Timers:CreateTimer(3.2, function()
								...
								visionTracer:MoveToPosition(MAIN_HERO_TABLE[1]:GetAbsOrigin())
								  GameRules.siegeQuest = SpawnEntityFromTableSynchronous( "quest", { name = "QuestSiege", title = "#quest_defend_keep" } )
								  GameRules.subSiegeQuest = SpawnEntityFromTableSynchronous( "subquest_base", {
								    show_progress_bar = true,
								    progress_bar_hue_shift = -119
								    })
								  GameRules.siegeQuest:AddSubquest( GameRules.subSiegeQuest )

								  -- text on the quest timer at start
								  GameRules.siegeQuest:SetTextReplaceValue( QUEST_TEXT_REPLACE_VALUE_CURRENT_VALUE, 0 )
								  GameRules.siegeQuest:SetTextReplaceValue( QUEST_TEXT_REPLACE_VALUE_TARGET_VALUE, 300 )

								  -- value on the bar
								  GameRules.subSiegeQuest:SetTextReplaceValue( SUBQUEST_TEXT_REPLACE_VALUE_CURRENT_VALUE, 0 )
								  GameRules.subSiegeQuest:SetTextReplaceValue( SUBQUEST_TEXT_REPLACE_VALUE_TARGET_VALUE, 300 )
...
function Dungeons:InitializeSandTomb()
	...
	ghostAbil = ghost:FindAbilityByName("npc_abilities"):ApplyDataDrivenModifier(ghost, ghost, "modifier_rare_ghost_thinker", {duration = 99999})	
 	...
 	Dungeons:SpawnDungeonUnit("spine_hermit", Vector(13312, -9984), 1, 7, 9, "bristleback_bristle_attack_18", Vector(1 ,1), false, nil)
...
function Dungeons:AggroUnit(caster)
	...
	if not aggrod then
		local ability = caster:AddAbility("dungeon_creep_aggroed")
	else
		local ability = aggrod
	end
	if ability then
		ability:SetLevel(1)
	end
