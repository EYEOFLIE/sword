...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorDismember,Behaviorself_haste, BehaviorThrowHook, BehaviorRunAway,BasicSkill } ) 
end
...
function Behaviorself_haste:Evaluate()
	print("22222222222")
	self.dismemberAbility = thisEntity:FindAbilityByName("nyx_assassin_mana_burn")
	local target
	local desire = 0
...
function Behaviorself_haste:Begin()
	self.endTime = GameRules:GetGameTime() + 5
...
BehaviorDismember.Continue = Behaviorself_haste.Begin --if we re-enter this ability, we might have a different target; might as well do a full reset

function Behaviorself_haste:Think(dt)
	if not self.target:IsAlive() then
		self.endTime = GameRules:GetGameTime()
		return
	end
...
function BasicSkill:Evaluate()
	local desire = 0
	print("templar_assassin_refraction skill")

	-- let's not choose this twice in a row
	print(AICore.currentBehavior)
	if AICore.currentBehavior == self then return desire end

	self.baseAbility = thisEntity:FindAbilityByName( "templar_assassin_refraction" )
...
function BasicSkill:Begin()
	print("templar_assassin_refraction")
	self.endTime = GameRules:GetGameTime() + 1
		self.order =
		{
			UnitIndex = thisEntity:entindex(),
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorDismember, Behaviorself_haste,BehaviorThrowHook, BehaviorRunAway,BasicSkill }
